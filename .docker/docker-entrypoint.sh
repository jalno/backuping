...
then
	...
	if [ -n "$BACKUPING_BACKUP_CRON_EXPRESSION" ]; then
		...
		echo "$BACKUPING_BACKUP_CRON_EXPRESSION source /backuping-env.save && $H_PHP_BIN $BACKUPING_BACKUPING_INDEX_FILE --process=$BACKUPING_BACKUPING_PROCESS@backup ${BACKUPING_BACKUP_CRON_VERBOSE:+--verbose}";
...
then
	/bin/echo -e "[Backuping]: action is: '$1'";
	echo "$H_PHP_BIN $BACKUPING_BACKUPING_INDEX_FILE --process=$BACKUPING_BACKUPING_PROCESS@$1"
else
	eval $1
fi
